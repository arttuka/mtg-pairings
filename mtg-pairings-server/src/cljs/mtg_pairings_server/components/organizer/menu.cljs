...
(defn tournament-menu-item [{:keys [selected on-click item]}]
  [menu-item {:selected selected
              :on-click on-click}
   [list-item-text {:primary   (:name item)
...
(def styled-button ((with-styles (fn [{:keys [spacing]}]
                                   {:root {:flex   "0 1 120px"
                                           :margin (spacing 0 1)}}))
                    button))

(defn menu []
  (with-let [page (subscribe [::common-subs/page])
             ...
             clocks (subscribe [::subs/organizer :clock])
             minutes (atom 50)
    ...
    (let [multi? (nil? (:id @page))]
      [app-bar {:color    :default
       ...
       [toolbar
        ...
        [styled-button {:on-click #(dispatch [::events/organizer-mode :seatings])
                        :color    :primary
                        :variant  (if @new-seatings :contained :outlined)
                        :disabled (not @seatings)}
         "Seatings"]
        [styled-button {:on-click #(dispatch [::events/organizer-mode :clock])
                        :variant  :outlined
                        :color    :primary}
         "Kello"]
        ...
        [clock-buttons {:set-clock      set-clock
                        ...
                        :selected-clock (or (some? @selected-clock)
                                            (= 1 (count @clocks)))}]]])))
